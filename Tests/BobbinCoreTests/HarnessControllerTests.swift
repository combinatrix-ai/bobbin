...
final class HarnessControllerTests: XCTestCase {
    ...
    func testReauthorizationServerErrorFromActionPromotesAuthState() async throws {
        ...
        controller.send("requires a fresh session", in: threadID)
        await waitUntil("failed turn reports reauthorization") {
            controller.store.state.threads.first(where: { $0.id == threadID })?.status == .failed
        }

        XCTAssertEqual(controller.authState, .reauthorizationRequired)
        XCTAssertEqual(controller.serverState, .ready)
        XCTAssertEqual(controller.lastError, "  [REAUTHORIZATION IS REQUIRED!]  ")
    }

    func testGenericServerErrorDoesNotPromoteAuthState() async throws {
        let fixture = try Fixture()
        defer { fixture.cleanup() }
        ...
        client.responseHandler = { method, _ in
            if method == "turn/start" {
                throw HarnessError.serverError(code: -1, message: "turn rejected")
            }
            return client.defaultResponse(for: method)
        }
        let controller = try HarnessController(testPaths: fixture.paths, appServerClient: client)
        ...
        controller.send("a normal failed turn", in: threadID)
        await waitUntil("failed turn reports generic error") {
            controller.store.state.threads.first(where: { $0.id == threadID })?.status == .failed
        }

        XCTAssertFalse(controller.authState.isReauthorizationRequired)
        XCTAssertEqual(controller.authState, .authenticated(.deviceAuth))
        XCTAssertEqual(controller.lastError, "turn rejected")
    }

    func testAccountUpdatedNullAuthModeRequiresReauthorization() throws {
        let fixture = try Fixture()
        defer { fixture.cleanup() }
        ...
        XCTAssertEqual(controller.authState, .authenticated(.apiKey))
    }
